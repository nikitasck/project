...
@section('content')
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0">Update product: {{$product->manufacture}} {{ $product->name }}</h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
          </div><!-- /.col -->
        </div><!-- /.row -->
                    ...
                    <form action="{{route('product.update', $product->id)}}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-10">
                              <select name = "category" class="form-control">
                                @foreach ($categories as $category)
                                    <option value ="{{ $category->id }}" @if($product->category_id === $category->id) selected="selected" @endif>{{$category->category}}</option>
                                @endforeach
                              </select>
                            </div>
                            <div class="col-2">
                          ...
                          <div class="form-group">
                            ...
                            @enderror
                            <input name="manufacture" value="{{ $product->manufacture }}" class="form-control" id="categoryInputText" placeholder="Enter product manufacture">
                        </div>
                        <div class="form-group">
                            @error('name')
                            <div class="alert alert-danger">{{ $message }}</div>
                            ...
                            <input type="text" name="name" value="{{ $product->name }}" class="form-control" id="categoryInputText" placeholder="Enter product name">
                        </div>
                        <div class="form-group">
                            @error('description')
                            <div class="alert alert-danger">{{ $message }}</div>
                            ...
                            <input type="text" name="description" value="{{ $product->description }}" class="form-control" id="categoryInputText" placeholder="Enter product description">
                        </div>
                        <div class="form-group">
                            @error('price')
                            <div class="alert alert-danger">{{ $message }}</div>
                            ...
                            <input type="text" name="price" value="{{ $product->price }}" class="form-control" id="categoryInputText" placeholder="Enter product price">
                        </div>
                            ...
                            <div class="col-10">
                              <select id="color" name="colors[]" class="form-control colors select2-hidden-accessible" multiple="" data-placeholder="Select a color" style="width: 100%;" data-select2-id="" tabindex="-1" aria-hidden="true">
                                @foreach ($colors as $color)
                                    <option value="{{ $color->id }}" data-select2-id="{{$color->id}}" 
                                      @foreach ($product->colors as $productColor) 
                                        @if($productColor->color === $color->color) 
                                          selected="selected" 
                                        @endif 
                                      @endforeach>
                                      {{$color->color}}
                        ...
                        <div class="form-group">
                          <label>Sizes</label>
                          <p>If you want to update product size choose it or add new one.</p>
                          <div class="row">
                            <div class="col-10">
                              <select id="size" name="sizes[]" class="form-control sizes select2-hidden-accessible" multiple="" data-placeholder="Select " style="width: 100%;" data-select2-id="" tabindex="-1" aria-hidden="true">
                                @foreach ($sizes as $size)
                                  <option value="{{ $size->id }}" data-select2-id="{{$size->id}}" 
                                    @foreach ($product->sizes as $productSize) 
                                      @if($productSize->size === $size->size) 
                                        selected="selected" 
                                      @endif 
                                    @endforeach>
                                      {{$size->size}}
                            ...
                            <div class="col-10">
                              <select id="storage" name="storages[]" class="form-control storages select2-hidden-accessible" multiple="" data-placeholder="Select " style="width: 100%;" data-select2-id="" tabindex="-1" aria-hidden="true">
                                @foreach ($storages as $storage)
                                  <option value="{{ $storage->id }}" data-select2-id="{{$storage->id}}" 
                                    @foreach ($product->storages as $productStorage) 
                                      @if($productStorage->storage_size === $storage->storage_size) 
                                        selected="selected" 
                                      @endif 
                                    @endforeach>
